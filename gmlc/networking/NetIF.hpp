...
#pragma GCC diagnostic ignored "-Wstrict-aliasing"
     ...
     * @param sa_len the length of the sockaddr (primarily for systems that
     * don't support inet_ntop)
     * @return an IP address in text form, or an empty string if conversion to
     * text failed
     */
    inline std::string addressToString(struct sockaddr* addr, int sa_len)
    {
        ...
        if (getnameinfo(
                addr, sa_len, addr_str, NI_MAXHOST, 0, 0, NI_NUMERICHOST) !=
            0) {
            return std::string();
        }
#else
...
#pragma GCC diagnostic pop
    ...
    /**
     * a helper function to free the memory allocated to store a set of
     * interface addresses
     * @param addrs a pointer to the allocated address structure (type depends
     * on OS)
     */
    ...
    /**
     * a helper function to get a list of all interface adapters using OS system
     * calls. the returned pointer from addrs must be freed with a call to
     * freeAddresses.
     * @param family type of adapter addresses to get on Windows; one of AF_INET
     * (IPv4), AF_INET6 (IPv6), or AF_UNSPEC (both)
     * @param[out] addrs pointer to a pointer to be filled in with the address
     * of the allocated address structure; must be freed when done by calling
     * freeAddresses
        ...
        do {
            *addrs = reinterpret_cast<IP_ADAPTER_ADDRESSES*>(
            ...
            rv = GetAdaptersAddresses(
                family, GAA_FLAG_INCLUDE_PREFIX, NULL, *addrs, &bufLen);
    ...
    /**
     * a helper function to get the underlying socket address structure based on
     * OS
     * @param addr a pointer to an address structure for an (unicast)
     * interface/adapter
     * @return a socket address structure containing the IP address information
     * for an interface
     */
    ...
    /**
     * a helper function to get the underlying socket address length on systems
     * where it is needed
     * @param addr a pointer to an address structure for an (unicast)
     * interface/adapter
     ...
     * @param family specify the type of address desired on non-Windows systems;
     * one of AF_INET (IPv4), AF_INET6 (IPv6), or AF_UNSPEC (both)
     * @param addrs a pointer to an addresses structure for an interface/adapter
     * list
     * @return the next interface/adapter in the addresses list (that matches
     * the family given for non-Windows systems), or nullptr if there is no next
     * address
     */
    inline IF_ADDRS_UNICAST getNextAddress(int family, IF_ADDRS_UNICAST addrs)
    {
        ...
        while (next != NULL) {
            ...
            // Skip if a specific IP family was requested and the family doesn't
            // match
    ...
    /**
     * get all addresses associated with network interfaces on the system of the
     * address family given.
     * @param family the type of IP address to return; one of AF_INET (IPv4),
     * AF_INET6 (IPv6), or AF_UNSPEC (both)
    ...
    inline std::vector<std::string> getInterfaceAddresses(int family)
    {
        ...
        auto addrs = allAddrs;
            ...
            for (auto a = addrs; a != NULL; a = getNextAddress(family, a)) {
                std::string ipAddr =
                    addressToString(getSockAddr(a), getSockAddrLen(a));
                if (!ipAddr.empty()) {
    ...
    /**
     * returns a list of all IPv4 addresses associated with network interfaces
     * on the system.
    ...
    /**
     * returns a list of all IPv6 addresses associated with network interfaces
     * on the system.
    ...
    /**
     * returns a list of all IPv4 and IPv6 addresses associated with network
